...
let p_parouv : 't analist = fun l -> match l with
  | '(' :: l -> l
  | _ -> raise Echec

let p_parfer : 't analist = fun l -> match l with
  | ')' :: l -> l
  | _ -> raise Echec

let p_x : 't analist = fun l -> match l with
  | 'x' :: l -> l
  | _ -> raise Echec
...
let terminal_cond (p : char -> bool) : 't analist = fun l -> match l with
  | x :: l when p x -> l
  | _ -> raise Echec
...
let return (res : 'r) : 'r r't analist = fun l -> res, l
...
let p_S0 : 't analist = terminal 'x'

let p_S1 : 't analist = fun l ->
...
let p_S2 : 't analist = terminal 'x'

let p_S : 't analist = fun l ->
...
let rec p_S : 't analist =
  let p_S1 : 't analist = fun l ->
    let l = terminal '(' l in
    let l = p_S l in
    let l = terminal ')' l in
    l
  and p_S2 : 't analist = fun l ->
...
let rec p_S : ast r't analist =
  let p_S1 : ast r't analist = fun l ->
    let l = terminal '(' l in
    let a, l = p_S l in
    let l = terminal ')' l in
    Pa (a), l
  and p_S2 : ast r't analist = fun l ->
    let l = terminal 'x' l in
    Fin, l
  in fun l ->
     try p_S1 l with
